...
// prepare note object
        val note = Note(
            ...
            cursor.getString(
                cursor.getColumnIndexOrThrow(NotesData.COLUMN_NOTE_DESCRIPTION)),
            cursor.getString(cursor.getColumnIndexOrThrow(NotesData.COLUMN_TIMESTAMP))
        )
        note.isReminder = (cursor.getInt(
            cursor.getColumnIndexOrThrow(NotesData.COLUMN_NOTE_SET_REMINDER))==1)
        note.remindertime =
...
// Select All Query
            val selectQuery = "SELECT * FROM " +
                    NotesData.TABLE_NAME.toString() + " ORDER BY " +
